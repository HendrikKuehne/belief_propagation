...
def contract_edge(
    ...
    """
    Contracts the edge `(node1, node2, key)` in the tensor network `G`.
    `G` is modified in-place. `node2` is removed, and the new node
    assumes the label `node1`.
    """
    # Sanity check.
    if sanity_check: assert network_intact_check(G)

    if node1 == node2:
        # (node1, node2) is a trace edge.
        ...
        for _, neighbor, key1 in G.edges(node1, keys=True):
            if G[node1][neighbor][key1]["trace"]:
                ...
                G[node1][neighbor][key1]["indices"] = {
                    update_leg(i)
                    for i in G[node1][neighbor][key1]["indices"]
                }
            else:
    ...
    for _, neighbor, key2 in G.edges(node2, keys=True):
        if node2 == neighbor:
            ...
            keyvals = {
                ...
                "indices": {
                    update_leg2(i)
                    for i in G[node2][neighbor][key2]["indices"]
                },
            }
            # Adding the new edge.
            ...
            keyvals = {
                ...
                "trace": True,
                "indices": {
                    G[node2][neighbor][key2]["legs"][node1],
                    # This leg was updated when the legs incident to node1 were
                    # updated.
...
def merge_edges(
    ...
    """
    Merges all parallel edges between `node1` and `node2`. `G` is
    modified in-place.
    """
    ...
    axes1 = []
    """The `axes` argument for `np.transpose`, together with `legs1`."""
    axes2 = []
    """The `axes` argument for `np.transpose`, together with `legs2`."""
    legs1 = list(range(G.nodes[node1]["T"].ndim))
    """The `axes` argument for `np.transpose`, together with `axes1`."""
    legs2 = list(range(G.nodes[node2]["T"].ndim))
    """The `axes` argument for `np.transpose`, together with `axes2`."""
    ...
    # The tensors of node1 and node2 must be transposed s.t. duplicate edges
    ...
    # Transposing the tensors, s.t. the parallel edges connect to the leading
    # dimensions.
    ...
    for _, neighbor, key in G.edges(node1, keys=True):
        if G[node1][neighbor][key]["trace"]:
            G[node1][neighbor][key]["indices"] = {
                leg + legshift1(leg)
                for leg in G[node1][neighbor][key]["indices"]
            }

        else:
            leg = G[node1][neighbor][key]["legs"][node1]
            G[node1][neighbor][key]["legs"][node1] += legshift1(leg)

    for _, neighbor, key in G.edges(node2, keys=True):
        if G[node2][neighbor][key]["trace"]:
            G[node2][neighbor][key]["indices"] = {
                leg + legshift2(leg)
                for leg in G[node2][neighbor][key]["indices"]
            }

        else:
            leg = G[node2][neighbor][key]["legs"][node2]
            G[node2][neighbor][key]["legs"][node2] += legshift2(leg)

    # adding a new edge
    G.add_edge(
        node1,
        node2,
        legs={node1: 0, node2: 0},
        trace=False,
        indices=None
    )
...
def expose_edge(
    ...
    connecting them sums over their trailing dimensions. `G` is modified
    in-place.
    """
    if sanity_check:
    ...
    # Permuting the legs of u and v such that edge (u,v) corresponds to
    # the trailing index in nodes u and v.
...
def feynman_cut(
    ...
    """
    Executes a Feynman-cut on the network `G`, and returns a list that
    contains the respective networks. Assumes that the network is intact
    and message-ready.
    """
...
def construct_initial_messages(
    ...
    """
    Initializes messages one the edges of `G`. Random initialisation
    except for leaf nodes, where the initial value is the tensor of the
    leaf node. `G` is modified in-place.
    """
    # Sanity check.
    ...
    for node1, node2 in G.edges():
        ...
        for sender, receiver in itertools.permutations((node1, node2), r=2):

            if len(G.adj[sender]) == 1:
                # Message from leaf node.
                G[node1][node2][0]["msg"][receiver] = G.nodes[sender]["T"]
                if normalize:
                    norm = np.sum(G.nodes[sender]["T"])
                    G[node1][node2][0]["msg"][receiver] /= norm

            else:
                iLeg = G[node1][node2][0]["legs"][receiver]
                chi = G.nodes[receiver]["T"].shape[iLeg]
                # Initialization with normalized vector
                msg = np.random.normal(size=(chi,))
                G[node1][node2][0]["msg"][receiver] = msg
                if normalize:
                    G[node1][node2][0]["msg"][receiver] /= np.sum(msg)
...
def dummynet1(real: bool = False, chi: int = 3) -> tuple[nx.MultiGraph, float]:
    ...
    T = lambda size: rng.standard_normal(size) if real else crandn(size)
    G.nodes[0]["T"] = T(size=(chi,))
    G.nodes[1]["T"] = T(size=(chi, chi, chi))
    G.nodes[2]["T"] = T(size=(chi, chi, chi))
    G.nodes[3]["T"] = T(size=(chi,))
    G.nodes[4]["T"] = T(size=(chi,))
    G.nodes[5]["T"] = T(size=(chi,))
...
def dummynet3(real: bool = False, chi: int = 3) -> tuple[nx.MultiGraph, float]:
    ...
    T = lambda size: rng.standard_normal(size) if real else crandn(size)
    G.nodes[0]["T"] = T(size=(chi,))
    G.nodes[1]["T"] = T(size=(chi, chi, chi, chi))
    G.nodes[2]["T"] = T(size=(chi,))
...
def dummynet4(real: bool = False, chi: int = 3) -> tuple[nx.MultiGraph, float]:
    ...
    G.nodes[0]["T"] = T(size=(chi,))
    G.nodes[1]["T"] = T(size=(chi, chi, chi, chi))
    G.nodes[2]["T"] = T(size=(chi, chi, chi, chi, chi))
    G.nodes[3]["T"] = T(size=(chi, chi, chi))
    G.nodes[4]["T"] = T(size=(chi, chi, chi))
    G.nodes[5]["T"] = T(size=(chi, chi))
    G.nodes[6]["T"] = T(size=(chi, chi))
...
def dummynet5(real: bool = False, chi: int = 3) -> tuple[nx.MultiGraph, float]:
    ...
    T = lambda size: rng.standard_normal(size) if real else crandn(size)
    G.nodes[0]["T"] = T(size=(chi))
    G.nodes[1]["T"] = T(size=(chi,chi))
    G.nodes[2]["T"] = T(size=(chi,chi,chi))
    G.nodes[3]["T"] = T(size=(chi,chi))
    G.nodes[4]["T"] = T(size=(chi,chi,chi))
    G.nodes[5]["T"] = T(size=(chi,chi))
    G.nodes[6]["T"] = T(size=(chi))
...
def grid_net(
    ...
    r"""
    Construct random tensors forming a network on a two-dimensional
    square lattice. `chi` is the bonding dimension. Returns a list that
    contains the grid, as well as a `nx.MultiGraph`.
    ...
    for i in range(height):
        ...
        for j in range(width):
            ...
            # Adding edges.
            if i != height - 1: G.add_edge(
                i * width + j,
                (i + 1) * width + j,
                legs={i * width + j: 3, (i + 1) * width + j: 2},
                trace=False,
                indices=None
            )
            if j != width - 1: G.add_edge(
                i * width + j,
                i * width + j + 1,
                legs={i * width + j: 1, i * width + j + 1: 0},
                trace=False,
                indices=None
            )
        tensors.append(row)
    ...
    for i in range(height):
        ...
        G.add_edge(
            outer_counter,
            i * width,
            legs={i * width: 0, outer_counter: 0},
            trace=False,
            indices=None
        )
        outer_counter += 1
        ...
        G.add_edge(
            outer_counter,
            (i + 1) * width - 1,
            legs={(i + 1) * width - 1: 1, outer_counter: 0},
            trace=False,
            indices=None
        )
        outer_counter += 1
    ...
    for j in range(width):
        ...
        G.add_edge(
            outer_counter,
            (height - 1) * width + j,
            legs={(height - 1) * width + j: 3, outer_counter: 0},
            trace=False,
            indices=None
        )
        outer_counter += 1
