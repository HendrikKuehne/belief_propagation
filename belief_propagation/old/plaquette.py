"""
Code from Christian Mendl - the base on which I will build. Not to be
modified in any substantial way, for reference.
"""
import numpy as np
import matplotlib.pyplot as plt
import warnings

from belief_propagation.utils import crandn
...
def construct_network(
    ...
    square lattice. `chi` is the bonding dimension.
...
def contract_network(tensors: list):
    r"""
    Contract a tensor network on a two-dimensional square lattice. The
    first row is interpreted as an MPO, and subsequent rows are
    contracted into it.
    ...
    if not a.shape == (1, 1, 1, 1):
        warnings.warn(
            "Unexpected shape after contracting the network; continuing.",
            RuntimeWarning
        )
...
def block_bp(tensors: list, verbose: bool = False):
    ...
    """
    # s0: Turning the upper-left 3x3 plaquettes into one plaquette; first, we
    # construct an MPO.
    mpo = tensors[0][:3]
    ...
    for a in mpo[1:]:
        s0 = np.einsum(
            ...
            s0.shape[0],
            a.shape[1],
            s0.shape[2] * a.shape[2],
            s0.shape[3] * a.shape[3]
        ))
    ...
    for a in (tensors[1][3], tensors[2][3]):
        s1 = np.einsum(
            ...
            s1.shape[0] * a.shape[0],
            s1.shape[1] * a.shape[1],
            s1.shape[2],
            a.shape[3]
        ))
    ...
    for a in tensors[3][1:3]:
        s2 = np.einsum(
            ...
            s2.shape[0],
            a.shape[1],
            s2.shape[2] * a.shape[2],
            s2.shape[3] * a.shape[3]
        ))
...
def construct_initial_messages(tensors:list):
    """
    Construct initial message vectors for a tensor network on a
    two-dimensional square lattice.

    The returned lists each contain one message vector for every tensor
    in the network. The message lists contain incoming messages for the
    respective grid point; `msg_in_l[i][j]` contains the message that is
    passed to `tensors[i][j]` from the left. The message vectors
    themselves are initialised as containing only ones, and normalized
    to unity.
    """
    # Initialization.
    height = len(tensors)
    ...
    for i in range(height):
        ...
        for j in range(width):
            row_in_l.append(
                np.ones(tensors[i][j].shape[0]) / tensors[i][j].shape[0]
            )
            row_in_r.append(
                np.ones(tensors[i][j].shape[1]) / tensors[i][j].shape[1]
            )
            row_in_u.append(
                np.ones(tensors[i][j].shape[2]) / tensors[i][j].shape[2]
            )
            row_in_d.append(
                np.ones(tensors[i][j].shape[3]) / tensors[i][j].shape[3]
            )
...
def message_passing_step(
    ...
    """
    Perform a message passing update step, using the provided incoming
    messages from the respective directions. Messages from the boundary
    have to be set to the dummy vector [1].
    """
    # Initialization.
    height = len(tensors)
    width  = len(tensors[0])
    ...
    for i in range(height):
        ...
        row_out_d = []
        for j in range(width):
            # The outcoming message on one side is the result of absorbing all
            # incoming messages on all other sides into the tensor.
            msg_l = np.einsum(
                tensors[i][j], (0, 1, 2, 3),
                msg_in_r[i][j], (1,),
                msg_in_u[i][j], (2,),
...
def message_passing_iteration(
    ...
    """
    Perform a message passing iteration. Algorithm taken from Kirkley,
    ...
    for n in range(1, numiter + 1):
        msg_in_l_next, msg_in_r_next, msg_in_u_next, msg_in_d_next = message_passing_step(
            tensors, msg_in_l, msg_in_r, msg_in_u, msg_in_d
        )
        eps = 0
        ...
        for i in range(height):
            for j in range(width):
                eps = max(
                    eps,
                    np.linalg.norm(msg_in_l_next[i][j] - msg_in_l[i][j])
                )
                eps = max(
                    eps,
                    np.linalg.norm(msg_in_r_next[i][j] - msg_in_r[i][j])
                )
                eps = max(
                    eps,
                    np.linalg.norm(msg_in_u_next[i][j] - msg_in_u[i][j])
                )
                eps = max(
                    eps,
                    np.linalg.norm(msg_in_d_next[i][j] - msg_in_d[i][j])
                )
        eps_iter.append(eps)
...
def normalize_messages(
    ...
    traveling along the same bond but in opposite directions is one.
    """
    # Initialization.
    height = len(msg_in_l)
    ...
    for i in range(height):
        ...
        for j in range(1, width):
            row_in_l.append(
                msg_in_l[i][j] / np.sqrt(np.abs(np.dot(
                    msg_in_l[i][j],
                    msg_in_r[i][j - 1]
                )))
            )
        ...
        for j in range(width - 1):
            row_in_r.append(
                msg_in_r[i][j] / np.sqrt(np.abs(np.dot(
                    msg_in_r[i][j],
                    msg_in_l[i][j + 1]
                )))
            )
    ...
    for i in range(1, height):
        ...
        for j in range(width):
            row_in_u.append(
                msg_in_u[i][j] / np.sqrt(np.abs(np.dot(
                    msg_in_u[i][j],
                    msg_in_d[i - 1][j]
                )))
            )
    ...
    for i in range(height - 1):
        ...
        for j in range(width):
            row_in_d.append(
                msg_in_d[i][j] / np.sqrt(np.abs(np.dot(
                    msg_in_d[i][j],
                    msg_in_u[i + 1][j]
                )))
            )
...
def main():
    ...
    msg_in_l, msg_in_r, msg_in_u, msg_in_d = normalize_messages(
        msg_in_l, msg_in_r, msg_in_u, msg_in_d
    )

    # Contract incoming messages with tensors.
    cntr = contract_tensors_messages(
        s_tensors, msg_in_l, msg_in_r, msg_in_u, msg_in_d
    )
    print("cntr:")
    print(cntr)
