"""
All the different algorithms that are implemented, contained in single
functions that accept a network and give back a contraction value.
"""
...
from belief_propagation.old import BP, loopyNBP, networks, plaquette
...
def BP_routine_blocking(
    ...
    """
    Complete pipeline of the tensor network code on a grid, including
    blocking.
    """
...
def loopyNBP_routine(
        G: nx.MultiGraph,
        r: int = 2,
        num_iter: int = 30
    ) -> float:
    """
    Complete pipeline of the tensor network code, including neighborhood
    grouping.
    """
...
def loopyNBP_feynman_routine(
    ...
    """
    Complete pipeline of the tensor network code, including
    neighborhoods and a feynman cut.
    """
    neighborhood_list = loopyNBP.construct_neighborhoods(G,r)
    ...
    for node1,node2 in G.edges():
        size = G.nodes[node1]["T"].shape[G[node1][node2][0]["legs"][node1]]
        G[node1][node2][0]["size"] = size

    # Sorting the edges based on their size.
    sorted_edge_list = sorted(
        G.edges(),
        key=lambda edge: G[edge[0]][edge[1]][0]["size"],
        reverse=True
    )
    ...
    for Gcut in cut_graphs:
        cntr += BP_routine(Gcut, num_iter=num_iter)
...
def PQ_routine_blocking(tensors: list, num_iter: int = 30) -> float:
    """
    Complete pipeline of the plaquette code with block belief
    propagation.
    """
    s_tensors = plaquette.block_bp(tensors)
    return PQ_routine(s_tensors, num_iter=num_iter)
